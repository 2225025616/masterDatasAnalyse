...
# 导出再生过程数据
def outputRFBGInfo(sourceFile, outputFile):
    df = pd.DataFrame(
        columns=('time', 'ctwl/nm', 'thresh/dBm', 'notch/dB', 'reflection/%'))
    regenerationSpec = readSpec(sourceFile[1], sourceFile[2], sourceFile[3])
...
# 热电偶数据 温度时间整合并导出
def outputTempTime(sourceFile, outputFile):
    timeDf = pd.DataFrame(columns=['time', 'temperature'])
    datetime = readTemp(sourceFile[0], sourceFile[1])
    timeDf["time"] = datetime[0]
    timeDf["temperature"] = datetime[2]
    timeDf.to_csv(outputFile)
